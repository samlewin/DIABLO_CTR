...
! This variable is used to add up scalar diagnostics
      real*8 thsum(0:NY+1), thsum_prime(0:NY+1)
! These variables are used to store and write 2D slices and 3D arrays
      ...
      real*8 varxyz(0:NXM,0:NZP-1,1:NY)
...
! Compute the 3D Spanwise Vorticity omega_y_xy
      F4(:,:,:) = 0.d0
...
! Convert to physical space
      call fft_xz_to_physical(CS1,F4,0,NY+1)    
...
! Calculate the perturbation energy at each level
...
! Calculate U_2D
      ...
      v_2d(i,j) = 0.0
        do k=0,NZP-1
          u_2d(i,j) = u_2d(i,j) + (U1(i,k,j)-ume(j))/float(NZ)
          v_2d(i,j) = v_2d(i,j) + (U2(i,k,j))/float(NZ)
        end do 
      end do
      end do
...
! Calculate KE_KH
      do j=0,NY
        ...
        ke_2d_tot(j)=0.
      do i=0,NXM 
        u_2d_ke(j)=u_2d_ke(j)+(u_2d(i,j))**2.
        v_2d_ke(j)=v_2d_ke(j)+0.5*((v_2d(i,j))**2. + 
     &                             (v_2d(i,j+1))**2.)
      end do
      end do

      call mpi_allreduce(mpi_in_place,u_2d_ke,NY+2,MPI_DOUBLE_PRECISION,
     &     MPI_SUM,MPI_COMM_Z,ierror)
      call mpi_allreduce(mpi_in_place,v_2d_ke,NY+2,MPI_DOUBLE_PRECISION,
     &     MPI_SUM,MPI_COMM_Z,ierror)
           
      do j=0,NY
        u_2d_ke(j)=u_2d_ke(j)/(float(NX))
        v_2d_ke(j)=v_2d_ke(j)/(float(NX))
        ke_2d_tot(j)=0.5*(u_2d_ke(j) + v_2d_ke(j))
      end do 
      
      CALL INTEGRATE_Y_VAR(ke_2d_tot,ke_2d_tot_bulk,MPI_COMM_Y)      
...
! Calculate KE_3D
      ...
      do j=0,NY
        ...
        ke_3d_tot(j)=0.
          do i=0,NXM 
          do k=0,NZP-1
            u_3d_ke(j)=u_3d_ke(j)+(U1(i,k,j) - ume(j) - u_2d(i,j))**2.
            w_3d_ke(j)=w_3d_ke(j)+(U3(i,k,j))**2.
            v_3d_ke(j)=v_3d_ke(j)+0.5*((U2(i,k,j)-v_2d(i,j))**2.+ 
     &                      (U2(i,k,j+1)-v_2d(i,j+1))**2.)
          end do
          end do
      end do 

      call mpi_allreduce(mpi_in_place,u_3d_ke,NY+2,MPI_DOUBLE_PRECISION,
     &     MPI_SUM,MPI_COMM_Z,ierror)
      call mpi_allreduce(mpi_in_place,v_3d_ke,NY+2,MPI_DOUBLE_PRECISION,
     &     MPI_SUM,MPI_COMM_Z,ierror)
      call mpi_allreduce(mpi_in_place,w_3d_ke,NY+2,MPI_DOUBLE_PRECISION,
     &     MPI_SUM,MPI_COMM_Z,ierror)

      do j=0,NY
        u_3d_ke(j)=u_3d_ke(j)/(float(NZ)*float(NX))
        w_3d_ke(j)=w_3d_ke(j)/(float(NZ)*float(NX))
        v_3d_ke(j)=v_3d_ke(j)/(float(NZ)*float(NX))
        ke_3d_tot(j)=0.5*(u_3d_ke(j) + v_3d_ke(j) + w_3d_ke(j))
      end do 

      CALL INTEGRATE_Y_VAR(ke_3d_tot,ke_3d_tot_bulk,MPI_COMM_Y) 
...
! Get the rms value
     ...
     &     MPI_SUM,MPI_COMM_Z,ierror)
      ...
      IF (MOD(TIME_STEP,SAVE_STATS_INT*4).EQ.0) THEN
       gname='time_red'
       call WriteHDF5_real(FNAME,gname,TIME)
      END IF
      ...
      gname='ke_tot_bulk'
      Diag_Scalar=ke_tot_bulk
      call WriteHDF5_real(FNAME, gname, Diag_Scalar)

      gname='ke_2d_tot_bulk'
      Diag_Scalar=ke_2d_tot_bulk
      call WriteHDF5_real(FNAME, gname, Diag_Scalar)

      gname='ke_3d_tot_bulk'
      Diag_Scalar=ke_3d_tot_bulk
      call WriteHDF5_real(FNAME, gname, Diag_Scalar)  
     ...
     &       MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_Z,ierror)
      ...
      if (MOVIE) then
        ...
        IF (RANKZ.EQ.RANKZMOVIE) THEN
            do I=0,NXM
            do J=1,NY
              varxy(i,j)=F4(i,NzMovie,j)
            end do
            end do
            GNAME='omega_z_xy'
            call writeHDF5_xyplane(FNAME,GNAME,varxy)
        END IF     
        IF (MOD(TIME_STEP,SAVE_STATS_INT*10).EQ.0) THEN
        if (USE_MPI) then
        call mpi_barrier(MPI_COMM_WORLD,ierror)
        end if
         IF (RANKZ.EQ.RANKZMOVIE) THEN
