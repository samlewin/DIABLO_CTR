...
C Tanh shear layer
        ...
        DO K=0,NZP-1
        DO J=0,NY
          U1(I,K,J)= !SIN(GZ((RANKZ)*NZP + K )) 
    !  &      + 0.5*SIN(GZ((RANKZ)*NZP + K )/100.d0)
    !  &    (ERF(GX(I))+ERF(30.0-GX(I)))
    !  &    (ERF(GX(I))+ERF(30.0-GX(I)))
    !  &    (1+EXP(-(7.3e-2*(GZ((RANKZ)*NZP + K )-15.0))**8))*
     &    TANH(14.28*GYF(J))!*COS(PI*GYF(J)/LY)!+
    !  &    0.2*SIN(2*PI*GZ((RANKZ)*NZP + K )/LZ)
    !  &    (1-0.25*EXP(-(7.3e-2*(GZ((RANKZ)*NZP + K )-15.0))**8))*
    !  &    TANH(GYF(J)/
    !  &    (1-0.25*EXP(-(7.3e-2*(GZ((RANKZ)*NZP + K )-15.0))**8)))
          U2(I,K,J)=0.d0
          U3(I,K,J)=0.d0
          ...
          END DO
       END DO
      else if (IC_TYPE_U.eq.4) then
C zero initial velocity (to be driven by some forcing)
        DO J=0,NY
          DO K=0,NZP-1
            DO I=0,NXM
              U1(I,K,J)=0.d0
...
C Add random noise in physical space
      ...
      DO J=0,NY+1
       DO K=0,NZP-1
         DO I=0,NXM
           CALL RANDOM_NUMBER(RNUM1)
           U1(I,K,J)=U1(I,K,J)+KICK*(RNUM1-0.5d0)*EXP(-0.01*GYF(J)**4)
           CALL RANDOM_NUMBER(RNUM1)
           U2(I,K,J)=U2(I,K,J)+KICK*(RNUM1-0.5d0)*EXP(-0.01*GYF(J)**4)
           CALL RANDOM_NUMBER(RNUM1)
           U3(I,K,J)=U3(I,K,J)+KICK*(RNUM1-0.5d0)*EXP(-0.01*GYF(J)**4)
         END DO
        END DO
      END DO
...
C Convert to Fourier space      
      ...
      CALL FFT_XZ_TO_FOURIER(TH,CTH,0,NY+1)
...
! Optionally, add random noise in Fourier space instead
      DO I=0,NXP-1
        DO J=1,NY
          DO K=0,TNKZ 
            K_H = sqrt(KX(I)**2+KZ(K)**2) 
            if ((KX(I).LT.10).and.(KZ(K).LT.50)
     &           .and.(KZ(K).GT.0)) then
C Now, give the velocity field a random perturbation
    !         CALL RANDOM_NUMBER(RNUM1)
    !         CALL RANDOM_NUMBER(RNUM2)
    !         CU1(I,K,J)=CU1(I,K,J)
    ! !  &       +KICK*CMPLX(2*(RNUM1-0.5d0),2*(RNUM2-0.5d0)) 
    !  &      +KICK*(KX(I)/30)**2*
    !  &      EXP(CMPLX(-(KX(I)/30)**2/2, (0.d0)))*
    !  &      EXP(CMPLX(0.d0, 2.0*RNUM2*PI))*
    !  &         EXP(-0.01*GYF(J)**2)
    !         CALL RANDOM_NUMBER(RNUM1)
    !         CALL RANDOM_NUMBER(RNUM2)
    !         CU2(I,K,J)=CU2(I,K,J)
...
!            K=0         
          ...
          !  IF (IC_TYPE_U.eq.3) THEN
             CU1(I,K,J)=CU1(I,K,J)
     &             +KICK*KZ(K)**(-0.5)*EXP(-(GYF(J))**2.d0)*
     &               CEXP(CMPLX(0.d0, 2.0*RNUM1*PI))
             CU2(I,K,J)=CU2(I,K,J)
     &             +2*KICK*KZ(K)**(-0.5)*EXP(-(GYF(J))**2.d0)*
     &               CEXP(CMPLX(0.d0, 2.0*RNUM2*PI))
             CU3(I,K,J)=CU3(I,K,J)
     &             +KICK*KZ(K)**(-0.5)*EXP(-(GYF(J))**2.d0)*
     &              CEXP(CMPLX(0.d0, 2.0*RNUM3*PI))
    !        ELSE
...
! As an example, initialize TH1 with a sine in x
       DO K=0,NZP-1
         ...
         END DO
       END DO
       ELSE IF ((IC_TYPE_TH.eq.1).or.(IC_TYPE_TH.eq.2)) THEN
...
! Linear profile with slope corresponding to upper value
         ...
         END DO
        END DO
        ELSE IF (IC_TYPE_TH.eq.3) then
! Tanh/erf profile 
       DO K=0,NZP-1
         DO I=0,NXM
           DO J=1,NY
             TH(I,K,J,N)=0.d0 !TANH(GYF(J))! ERF(GYF(J)) 
           END DO
         END DO
       END DO
       ELSE
