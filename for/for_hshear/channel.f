...
C For reference, see Numerical Renaissance (sections 13 and 10.4)
...
! Add the terms owing to the (vertical) system rotation (Coriolis terms)
...
! If a scalar contributes to the density, RI is not equal to zero and
...
C The subgrid scale stress is added to CFi:   CFi=CFi - d/dx_i tau_ij
      ...
      IF (LES.AND.((.NOT.CREATE_NEW_FLOW).OR.(TIME_STEP.GT.2000))) THEN
        ...
        DO K=0,TNKZ
          DO I=0,NXP-1
            ! IF ((RANKZ.NE.0).OR.(I.NE.0).or.(K.NE.0)) THEN
              CS1(I,K,J)=CS1(I,K,J)+RI(N)*CIKZ(K)*CTH(I,K,J,N)
...
C Read input file.
     ...
     &         STOP 'Wrong input data format input_chan'
      ...
      READ(11,*)
      READ(11,*) IC_TYPE_TH, IC_TYPE_U, KICK
      READ(11,*)
      READ(11,*) I_RO, OMEGA_N, AMP, DELTA_U, T_START
      READ(11,*)
