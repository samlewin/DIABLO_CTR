...
!     Close the properties for the dataspace creation and the writing
      ...
      INTEGER I,J,K,N
      REAL*8 RNUM1,RNUM2,RNUM3, K_H, DELTA
...
!     Close the properties for the reading
      ...
      !   END DO
      ! END DO
      DELTA=1.0 !14.28/non-dimensional wavelength of primary billow
      ! Convert to fourier space 
      call fft_xz_to_fourier(U1,CU1,0,NY+1)
      call fft_xz_to_fourier(U2,CU2,0,NY+1)
      call fft_xz_to_fourier(U3,CU3,0,NY+1)
      call fft_xz_to_fourier(THMEZ,CTHMEZ,0,NY+1)
      ! add noise
          DO I=0,NXP-1
          DO J=1,NY
          DO K=0,TNKZ
C Now, give the velocity field a random perturbation
    ...
    !  &       +KICK*CMPLX(2*(RNUM1-0.5d0),2*(RNUM2-0.5d0)) 
     &      +KICK*(KX(I)/2/DELTA)**2*
     &      EXP(CMPLX(-(KX(I)/2/DELTA)**2/2, (0.d0)))*
     &      EXP(CMPLX(0.d0, 2.0*RNUM2*PI))*
     &         EXP(-0.01*DELTA**2*GYF(J)**2)
            CALL RANDOM_NUMBER(RNUM1)
            CALL RANDOM_NUMBER(RNUM2)
            CU2(I,K,J)=CU2(I,K,J)
    !  &      +KICK*CMPLX(2*(RNUM1-0.5d0),2*(RNUM2-0.5d0)) 
...
!     CALL POISSON_P_PER
      ...
      SUBROUTINE ReadHDF5_interp(FNAME)
      use hdf5
      ...
      INTEGER NXLR, NZLR, NZPLR
      INTEGER I_LR, J_LR, K_LR

      REAL*8 tmp(NX,NY,NZP)
      REAL*8 tmp2(512, NY, 32)
c
c     HDF5 ------------------------------------------------------
c
c     Dataset names
...
C Initialize the random number generator
      ...
      dimsm(1:3) = (/NXLR,NY,NZPLR/)
      dimsf(1:3) = (/NXLR,(NY-1)*NPROCY+1,NZLR/)
...
!     Write the dataset collectively
     ...
     +        mem_space_id = memspace_id) !, xfer_prp = plist_id_w)
         ...
         do I=0,NXM 
            I_LR = I/2
            K_LR = K/2
            tmp(I,K,J) = 0.5*(tmp2(I_LR, K_LR, J)
     +                   +tmp2(I_LR+1, K_LR, J))
         end do
         end do
         end do
...
!     CALL POISSON_P_PER
      ...
      end subroutine ReadHDF5_interp
...
!     Write the dataset collectively
     ...
     &     aid,error)
      ...
      SUBROUTINE WriteHDF5_XYZ(FNAME,gname,var3d)
      use hdf5
...
! Sync the cores as a precaution (probably not necessary)
      ...
      SUBROUTINE WriteHDF5_real_long(FNAME,gname,Diag,Diag_size)
! This subroutine writes a single real scalar to the specified file
      use hdf5
      INCLUDE 'header'
...
!     Write the dataset collectively
     ...
     &     aid,error)
      ...
      !----*|--.---------.---------.---------.---------.---------.---------.-|-------|
      subroutine WriteStatH5_X(fname, gname, Diag)
      !----*|--.---------.---------.---------.---------.---------.---------.-|-------|
      use hdf5
     ...
     &                   aid, Error)
      ...
      call h5close_f(Error)
            ...
            !----*|--.---------.---------.---------.---------.---------.---------.-|-------|
      subroutine WriteStatH5_Z(fname, gname, Diag)
      !----*|--.---------.---------.---------.---------.---------.---------.-|-------|
      use hdf5
